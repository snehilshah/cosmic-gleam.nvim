...
-- Get the directory where the cosmic_gleam plugin is located
local function get_plugin_dir()
  return debug.getinfo(1).source:match('@?(.*/)'):gsub('/lua/cosmic_gleam/$', '')
end
...
-- Turn cosmic_gleam naming conventions into more common ANSI names
local function get_palette(variant)
  package.loaded['cosmic_gleam/palettes/' .. variant] = nil
  local colors = require('cosmic_gleam/palettes/' .. variant)
  -- stylua: ignore
  return {
    bg             = colors.a.bg,
    fg             = colors.a.fg,
  ...
  for _, variant in ipairs { 'dark', 'light' } do
    ...
    for term, attrs in pairs(terminals) do
      ...
      if term == 'windows_terminal' then
        ...
        fwrite(cs_fmt, string.format('%s/cosmic_gleam_%s_colorscheme%s', dir, variant, attrs.ext))
        fwrite(tm_fmt, string.format('%s/cosmic_gleam_%s_theme%s', dir, variant, attrs.ext))
      else
        local fmt = interpolate(attrs.template, palette)
        if term == 'foot' then
          fmt = fmt:gsub('#', '')
        end
        fwrite(fmt, string.format('%s/cosmic_gleam_%s%s', dir, variant, attrs.ext))
      end
    end

    fwrite(generate_iterm2(palette), string.format('%s/term/iterm2/cosmic_gleam_%s.itermcolors', get_plugin_dir(), variant))
    fwrite(vim.json.encode(palette), get_plugin_dir() .. string.format('/cosmic_gleam_%s.json', variant))
  end
end
